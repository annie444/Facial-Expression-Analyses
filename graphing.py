...
new_data = data.loc[
    ...
    & ((data.trial_type == "Sucrose") | (data.trial_type == "Airpuff"))
]

frame_trial_idx = new_data.groupby(
    ["mouse_list", "week_list", "trial_type", "trial_num"], group_keys=False
).apply(lambda x: (x.frame_list - x.frame_list.min()))
...
for d, df in enumerate(dfs):
    for point in ["eye", "ear", "whisker", "mouth", "nostril"]:
        for coord in ["_x", "_y"]:
            df_coord = (
                df.loc[:, "upper_eye_x":"outer_nostril_y"]
                .filter(like=point)
                .filter(like=coord)
...
for d, df in enumerate(dfs):
    ...
    if d == 0:
        mean_and_sem_df_csc = pd.concat(mean_and_sem, axis=1).sort_index(
            level="frame_trial_idx"
        )
    elif d == 1:
        mean_and_sem_df_cse = pd.concat(mean_and_sem, axis=1).sort_index(
            level="frame_trial_idx"
        )
    ...
    for trial in trials:
        for point in points:
            ...
            fig.add_traces(
                [
                    go.Scatter(
                        x=mean_and_sem_df_cse.xs(
                            (week, trial), level=("week_list", "trial_type")
                        )
                        .index.get_level_values("frame_trial_idx")
                        .to_list()[1:],
                        y=mean_and_sem_df_cse.xs(
                            (week, trial), level=("week_list", "trial_type")
                        )[(f"{point}_speed", "mean")].iloc[1:],
                        line=dict(color=colors[6]),
                        mode="lines",
                        name=f"CSE {point}",
                    ),
                    go.Scatter(
                        x=mean_and_sem_df_csc.xs(
                            (week, trial), level=("week_list", "trial_type")
                        )
                        .index.get_level_values("frame_trial_idx")
                        .to_list()[1:],
            ...
            fig.add_traces(
                [
                    go.Scatter(
                        ...
                        y=mean_and_sem_df_cse.xs(
                            (week, trial), level=("week_list", "trial_type")
                        )[(f"{point}_speed", "upper_sem")].iloc[1:],
                        mode="none",
                        hoverinfo="skip",
                        showlegend=False,
                        fill="tonexty",
                    ...
                    go.Scatter(
                        ...
                        y=mean_and_sem_df_cse.xs(
                            (week, trial), level=("week_list", "trial_type")
                        )[(f"{point}_speed", "lower_sem")].iloc[1:],
                        mode="none",
                        hoverinfo="skip",
            ...
            fig.update_layout(
                title=(f"Speed of {point} for Trial Type {trial} on Week {week}"),
                xaxis=dict(
                    title="sec",
            ...
            pval = stats.ttest_ind(
                data_cse[(data_cse.week_list == week) & (data_cse.trial_type == trial)][
                    f"{point}_speed"
                ].values,
                data_csc[(data_csc.week_list == week) & (data_csc.trial_type == trial)][
                    f"{point}_speed"
                ].values,
            )
...
for week in weeks:
    for trial in trials:
        ...
        for fig in [fig_cse, fig_csc]:
            ...
            fig.update_layout(
                title=(f"Average facial speed for Trial Type {trial} on Week {week}"),
                xaxis=dict(
                    title="sec",
                    tickvals=[0, cs_start, cs_end, 677],
                    ticktext=[-10, 0, 3, 13],
            ...
            for trial in cs.trial_type.unique():
                for point in points:
                    integral = integrate.trapz(
                        cs.loc[
                            (cs_start < cs["frame_trial_idx"])
                            & (cs["frame_trial_idx"] < (cs_end + 90))
                            & (cs["mouse_list"] == mouse)
                            & (cs["week_list"] == week)
                            & (cs["trial_type"] == trial),
                            [f"{point}_speed"],
                    ...
                    integrals[i].loc[
                        (f"{mouse}", f"{trial}", f"{point}"), f"{week}"
                    ] = integral
...
df_melt.columns = ["mouse_type", "mouse", "trial_type", "point", "week", "distance"]
...
sig_comps = [
    index.replace("C", "").replace("(", "").replace(")", "").replace(":", ", ")
...
for f, fig in enumerate([fig0, fig1, fig2]):
    for i in range(len(integrals)):
        if integrals.loc[:, "baseline":"ketamine"].iloc[i, :].name[
            2
        ] == "Airpuff" and f in [0, 1]:
            fig.add_trace(
                go.Scatter(
                    x=integrals.columns.tolist(),
                    y=integrals.loc[:, "baseline":"ketamine"]
                    .iloc[i, :]
                    .values.flatten(),
                    meta=[
                        integrals.loc[:, "baseline":"ketamine"].iloc[i, :].name[1],
                        integrals.loc[:, "baseline":"ketamine"].iloc[i, :].name[2],
                        integrals.loc[:, "baseline":"ketamine"].iloc[i, :].name[3],
                    ],
                    mode="lines+markers",
                    marker=dict(
                        color=colors[6]
                        if integrals.loc[:, "baseline":"ketamine"].iloc[i, :].name[0]
                        == "CSE"
                        else colors[5],
                        symbol="circle",
                        size=10,
                    ...
                    + "Mouse: %{meta[0]}<br>"
                    + "Trial: %{meta[1]}<br>"
                    + "Facial feature: %{meta[2]}<br>",
                    ids=["week 0", "week 1", "week 5", "week 6"],
                    legendgroup="Airpuff",
                    name="Airpuff CMS"
                    if integrals.loc[:, "baseline":"ketamine"].iloc[i, :].name[0]
                    == "CSE"
                    else "Airpuff Control",
                    showlegend=False if i not in [5, 39] else True,
                )
            )

        elif integrals.loc[:, "baseline":"ketamine"].iloc[i, :].name[
            2
        ] == "Sucrose" and f in [0, 2]:
            fig.add_trace(
                go.Scatter(
                    x=integrals.columns.tolist(),
                    y=integrals.loc[:, "baseline":"ketamine"]
    ...
    fig.update_layout(
        title=("Distance traveled from CS onset to three seconds after US end"),
...
table9.loc[["C(week)", "C(mouse_type)", "C(mouse_type):C(trial_type)"], :].to_csv(
...
mean_and_sem_dfs = pd.concat(
    [mean_and_sem_df_csc, mean_and_sem_df_cse],
    axis=0,
    keys=["CSC", "CSE"],
    names=["mouse_type"],
)
...
dat = pd.concat([data_csc, data_cse], axis=0, keys=["CSC", "CSE"], names=["mouse_type"])
...
for f, fig in enumerate([fig0, fig1, fig2]):
    for i in range(len(integrals)):
        ...
        if f == 0 and i < 4:
            ...
            n = (
                "CMS"
                if integrals.loc[:, "baseline":"ketamine"]
                .groupby(level=["mouse_type", "trial_type"])
                .apply("mean")
                .iloc[i, :]
                .name[0]
                == "CSE"
            ...
            t = (
                integrals.loc[:, "baseline":"ketamine"]
                .groupby(level=["mouse_type", "trial_type"])
                .apply("mean")
                .iloc[i, :]
                .name[1]
            ...
            fig.add_trace(
                go.Scatter(
                    x=integrals.loc[:, "baseline":"ketamine"]
                    .groupby(level=["mouse_type", "trial_type"])
                    .apply("mean")
                    .columns.tolist(),
                    y=integrals.loc[:, "baseline":"ketamine"]
                    ...
                    + "Trial: %{meta[1]}<br>",
                    ids=["week 0", "week 1", "week 5", "week 6"],
                    name=f"{n} {integrals.loc[:, 'baseline':'ketamine'].groupby(level=['mouse_type', 'trial_type']).apply('mean').iloc[i, :].name[1]}",
                )
            )
        ...
        if f in [1, 2] and i < 2:
            ...
            name = (
                integrals.xs("Sucrose", level="trial_type")
                .loc[:, "baseline":"ketamine"]
                .groupby(level=["mouse_type"])
                .apply("mean")
                .iloc[i, :]
                .name
                if f == 2
                else integrals.xs("Airpuff", level="trial_type")
                .loc[:, "baseline":"ketamine"]
                .groupby(level=["mouse_type"])
                .apply("mean")
                .iloc[i, :]
